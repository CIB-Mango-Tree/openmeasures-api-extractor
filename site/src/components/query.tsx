...
export function QueryBuilder(): ReactElement<FC> {
  ...
  const handleSearchTermAdd = (): void => {
    setSearchTerms((state: SearchTermMap): SearchTermMap => ({
      ...state,
      [Object.keys(state).length]: { modifier: '', term: '' }
    }));
  };
    ...
    <Card className="col-span-8">
      <CardHeader>
        ...
        <CardAction className="self-center">
          <Button onClick={handleClear} disabled={submitDisabled}>Clear All</Button>
        </CardAction>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit}>
          <FieldSet>
            ...
            <FieldGroup>
              ...
              {Object.entries(searchTerms).map((item: [string, SearchTermValues], key: number): ReactElement<FC> => {
                const index: number = parseInt(item[0]);
                const entry: SearchTermValues = item[1];

                if (index === 0) return <SearchTermInput key={key}
                  index={index}
                ...
                return <SearchTermInput key={key}
                  index={index}
                  disabled={submitDisabled}
                  modifier={entry.modifier}
                  term={entry.term}
              ...
              <Button type="button" variant="link" onClick={handleSearchTermAdd}>
                <SquarePlus />
              </Button>
            </FieldGroup>
          </FieldSet>
